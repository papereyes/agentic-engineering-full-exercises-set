...
The recorded first-attempt commits and patches above remain unchanged. Review found shell, native patch, symlink, approval-conversion, and audit-metadata bypasses. The final branch now requires a shell allow rule, resolves every native path and patch target, redacts all untrusted audit fields, and uses an exact external receipt for approval-only actions because Codex `PreToolUse` does not support `permissionDecision: ask`. `tests/review-regressions.test.mjs` executes the adapter as a real process and covers safe edits, protected reads, multi-file and moved patches, an actual symlink, approval before and after a receipt, unknown actions, and a canary in every action field. The fresh hook validation and all checks pass with exit code 0.
