...
|---|---|---|---|---|
| Important | `src/services/invitationService.ts:52` | Acceptance copied a non-normalized invitation email into the member. | Added `tests/acceptInvitationNormalization.test.ts` first, observed exit 1, then normalized once before member construction in implementation commit `ab46b7dcccf3d9e06b4e8fbfeff79f852566309b`. | Focused regression exits 0; `npm run test:invitations` and `npm run typecheck` exit 0. GitNexus upstream impact was LOW: one direct test-file caller and no affected process or module. |
