...
| The transformed queue must be saved, returned as a copy, and observed by later fetches and totals. | `bugfix-context-app/docs/current-sla-policy.md:30`; `bugfix-context-app/docs/workflow-api-contract.md:8` | Persistence, totals, and clone isolation are checked at `scripts/run-incident-tests.mjs:90`, `:111`, and `:118`. |
...
| The previous implementation is complete. | `bugfix-context-app/docs/previous-agent-progress.md:5` and `:13` | Current source retained all three defects, and the initial incident run failed 4 of 10 checks. |
