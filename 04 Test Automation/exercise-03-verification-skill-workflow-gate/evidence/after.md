...
The owner reported that a default verifier run wrote Maven `target` output into the repository and therefore failed the clean-state check. A fresh-checkout reproduction confirmed that this repository's minimal wrapper ignores `MAVEN_ARGS`, and that `-Dproject.build.directory` does not override the model value. The review fix adds an environment-activated Maven build-directory property in `pom.xml`; the corrected run sets `MAVEN_BUILD_DIRECTORY` under `/tmp`, leaves every protected check and release-gate step intact, and makes the clean-state claim specific to the recorded command.
