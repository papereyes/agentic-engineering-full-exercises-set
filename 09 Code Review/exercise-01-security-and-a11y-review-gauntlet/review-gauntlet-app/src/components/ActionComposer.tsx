...
export function ActionComposer({ item, onSave }: ActionComposerProps) {
  ...
  async function submit() {
    setSaving(true);
    try { await onSave({ owner, note, status }); }
  ...
  return (
    <section className="action-composer" aria-label="Action composer">
      ...
      <div className="review-preview">{note}</div>
      ...
      <button type="button" onClick={submit} disabled={saving || note.trim().length < 8}>{saving ? "Saving..." : "Save draft"}</button>
    </section>
  );
}
