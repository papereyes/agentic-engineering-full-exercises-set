...
describe("cache review regressions", () => {
  ...
  it("[CACHE-MUTATION] orders a copy without changing the imported fixture", async () => {
    ...
    try {
      ...
      expect(workItems.map(({ id }) => id)).toEqual(reversed.map(({ id }) => id));
      expect(loaded.map(({ dueInDays }) => dueInDays)).toEqual([...loaded].map(({ dueInDays }) => dueInDays).sort((a, b) => a - b));
    } finally {
      workItems.splice(0, workItems.length, ...original);
    }
  });

  it("[CACHE-LIFECYCLE] persists saved fields for the next load", async () => {
    const target = workItems[0];
    await saveAction(target.id, { owner: "New owner", note: "Persist this note", status: "In Review" });

    await expect(fetchWorkItems()).resolves.toEqual(expect.arrayContaining([
      expect.objectContaining({ id: target.id, owner: "New owner", note: "Persist this note", status: "In Review" }),
    ]));
  });

  it("[CACHE-EVIDENCE] leaves persisted workflow data unchanged", async () => {
